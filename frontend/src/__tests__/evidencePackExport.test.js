...
describe('evidencePackExport', () => {
  ...
  it('builds a structured markdown evidence pack with triage and case data', () => {
    ...
    expect(markdown).not.toContain('Manifest Audit Trail')
    expect(markdown).not.toContain('Audit trail')
    expect(markdown).not.toContain('Export compatibility')
    expect(markdown).not.toContain('导出清单')
    expect(markdown).not.toContain('Manifest 审计追踪')
    expect(markdown).not.toContain('审计追踪')
    expect(markdown).not.toContain('审计轨迹')
